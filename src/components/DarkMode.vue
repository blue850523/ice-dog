...
<template>
  <button
    class="h-8 w-8 cursor-pointer select-none rounded bg-[var(--color-theme-50)] hover:bg-[var(--color-theme-300)] transition-all"
    @click="changeTheme(systemMode === 'dark' ? 'light' : 'dark')"
  >
    <span v-show="systemMode === 'light'">
      <svg
        t="1684833132212"
        class="icon"
